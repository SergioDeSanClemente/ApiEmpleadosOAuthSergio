...
namespace ApiEmpleadosOAuth {
    public class Startup {
        ...
        public void ConfigureServices(IServiceCollection services) {
            string cadenaAzure = this.Configuration.GetConnectionString("Azure");
            services.AddTransient<EmpleadosRepository>();
            services.AddDbContext<EmpleadosContext>(options => options.UseSqlServer(cadenaAzure));
            ...
            services.AddSwaggerGen(c => {
                c.SwaggerDoc("v1", new OpenApiInfo { Title = "ApiEmpleadosOAuth", Version = "v1" });
            });
        }
